const listAccounts = [
  ...
  const listID = listAccounts.map(function (value) {
      delete value.id;
      value.isActive = "isActive" in value ? true : null;
      return value;
  })
  console.log(listID);
  const totalBalance = listAccounts.reduce((x,value) => x+value.balance,0)
  console.log("Tổng Balance = " + totalBalance);


  const statisticsByAge = listPersons.reduce(
    (x,t) => {
        if(t.age<18)  x.treCon ++;
        else if(t.age>60) x.nguoiGia ++;
        else x.thanhNien ++;
        return x;
      }, {
        treCon: 0,
